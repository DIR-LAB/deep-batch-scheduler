...
def load_policy(fpath, env_name, workload_file, itr='last'):
    ...
    get_action = lambda x : sess.run(action_op, feed_dict={model['x']: x.reshape(1, -1)})
    get_value = lambda x : sess.run(v_op, feed_dict={model['x']: x.reshape(1, -1)})
    get_logits = lambda x : sess.run(logits, feed_dict={model['x']: x.reshape(1, -1)})

    # initialize the environment from scratch
    env = gym.make(env_name)
    env.my_init(workload_file=workload_file)

    return env, get_action, get_value, get_logits
    ...
    for i in range(0, iters):
        ...
        while True:
            ...
            if d:
                model = 0 - r
                break
        print ("Effective:", effective_choice, "Ratio:", effective_choice / total)
...
if __name__ == '__main__':
    ...
    env, get_action, get_value, get_logits = load_policy(args.fpath, args.env, workload_file, 'last')
    run_policy(env, get_action, get_value, get_logits, args.len, args.iter)